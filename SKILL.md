...
3. **Cover 模式**：生成高点击率的 YouTube/公众号封面图
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--mode` | `article` | 模式：`article`（文章配图）、`slides`（PPT 信息图）或 `cover`（封面图） |
...
| `--save-config` | - | 保存当前参数到项目配置（`.smart-illustrator/config.json`） |
| `--save-config-global` | - | 保存当前参数到用户配置（`~/.smart-illustrator/config.json`） |
| `--no-config` | `false` | 临时禁用配置文件，只使用命令行参数 |

### 风格复用配置

通过配置文件保存常用参数，实现系列内容的风格一致性。
...
**参考图路径处理**：配置文件中的相对路径会相对于配置文件所在目录解析。
